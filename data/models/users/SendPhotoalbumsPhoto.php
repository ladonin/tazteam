<?php   
class UsersSendPhotoalbumsPhoto{
...
static public function setnameforoneimage($i,$id){//задаем имя подгруженной картинке ($i - какой номер из всех картинок массива темы редактируется сейчас), если есть id - значит номер в файловой системе будет id
	if (isset(GeneralImagesWork::$imagessource['source'][$i])){//если картинка с таким индексом есть
		foreach (GeneralImagesCalculate::$imagessizes_users_album as $key=>$value){//для каждого элемента массива экземпляров
		if ($id>0) {
...
static protected function update_photos_full_db($MSQLc){//обновляем фото (полностью и фото и описание) в БД
	...
	$query="UPDATE 
		...
		SET  		
			`dateloading`='".GeneralGlobalVars::$timeunix."',
...
static public function delete_messages_from_topic($MSQLc){//удалить все сообщения из темы
	...
	DELETE
	FROM
		registrated_users___photoalbums_photos_messages
	WHERE 	
	 	id_user='".UsersPhotoalbumsBase::$id_autor_topic."'
		AND
	 	id_album='".GeneralGetVars::$var4."'
...
static public function detect_flagonephoto($MSQLc){//проверяем - единственная ли фотография в теме или еще есть
	...
	if ($row['count']>1) {UsersPhotoalbumsBase::$flagonephoto=0;}
	else {UsersPhotoalbumsBase::$flagonephoto=1;}
	return $row['count'];}
...
static public function set_get_num_page($MSQLc){//обновляем глобальную переменную идентификатора номера фото
	GeneralGetVars::$num_page=UsersPhotoalbumsBase::$next_num_page_photo;}
...
static public function detect_next_id_photo_afterdelete($MSQLc){//узнаем id следующей фотографии, чтобы на неё перейти после удаления текущей
	...
	WHERE 
		...
		AND
	 	id_album='".GeneralGetVars::$var4."'
		AND 
		id_photo>'".$_POST['id_photo']."' LIMIT 1";
	$res=GeneralMYSQL::query($MSQLc,$query);		
	$row=GeneralMYSQL::fetch_array($res);
	GeneralMYSQL::free($res);
...
static public function delete_photo($MSQLc){//удаляем фото
	...
	if (UsersPhotoalbumsBase::detect_belong_topic_to_user()==true){//если тема принадлежит нам
		...
		if (UsersPhotoalbumsBase::$flagonephoto==0) {//если не последняя фотография в теме
			...
			UsersPhotoalbumsBase::detect_num_page_photo_from_id($MSQLc,self::$idnextphoto_afterdelete);//определяем по id на какую страницу идем дальше
...
static protected function insert_intophotos_main($MSQLc){//вставляем фото в БД частично
	$query="INSERT 
		...
		ELSE 1
		END) AS maxidkeyreal,
		".UsersPhotoalbumsBase::$id_autor_topic.",
		".UsersBase::return_dir_catalog(UsersPhotoalbumsBase::$id_autor_topic).",
		".GeneralGetVars::$var4.",
		".self::$idsendphoto."
		FROM registrated_users___photoalbums_photos";
...
static public function detectidforsendphoto($MSQLc){//вычисляем id записанного фото
	$query="SELECT id_photo FROM registrated_users___photoalbums_photos WHERE id_user='".UsersPhotoalbumsBase::$id_autor_topic."' AND id_album='".GeneralGetVars::$var4."' ORDER BY id_photo DESC LIMIT 1";
	$res=GeneralMYSQL::query($MSQLc,$query);
	$row=GeneralMYSQL::fetch_array($res);
	GeneralMYSQL::free($res);
	...
	if (UsersPhotoalbumsBase::detect_belong_topic_to_user()==true){//если тема принадлежит нам
		...
		if (GeneralImagesUpload::$statusimageattached==1){//если изображение приложено и проверено		
			...
			self::finish_update_photos_main($MSQLc);//дописываем фото в БД окончательно	
				UsersPhotoalbumsBase::detect_num_page_photo_from_id($MSQLc,self::$idsendphoto);//определяем номер страницы, на которую перейдем, по идентификатору фото
				self::set_get_num_page($MSQLc);//обновляем глобальную переменную идентификатора номера фото
				//GeneralGetVars::set_submit_vars();//перестраиваем заново переменные
				return true;}}}
	return false;}
