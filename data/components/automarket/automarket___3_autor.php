...
<td valign="top" width="103" align="left">	
	<a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']);?>"><img src="<?php echo(UsersBase::return_url_photo($row['t_gen_photo'],$row['t_dir_user']."/".$row['id_user']."_2.".$row['t_site_photo_format'],$row['t_sn_photo_url_from_small'],$row['t_sn_photo_url_from_huge'],0));?>" width="100" height="100" class="refimage" style="border-right:3px solid <?php if (UsersBase::$cur_user_online){?>#f09007<?php } else { ?>#eeeeee<?php } ?>"></a>
</td>
<td valign="top" align="left" class="padding_left_10 padding_right_10">
		<a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']);?>" class="link_lead "><?php echo(UsersMyData::return_name($row['t_login_user'],$row['t_mail_user'],$row['t_name_user'],$row['t_surname_user'],$row['t_login_status']));?></a>
		...
		<?php if ((UsersMyData::$identified==1)&&(UsersMyData::$id!=$row['id_user'])) { ?>
			<div class="v_i_s"></div>
			<a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']);?>/dialogs" class="btn btn-success btn-small "><?php echo(UsersBase::$array_buttons_to_userslists[1]);?></a>
		<?php } ?>
