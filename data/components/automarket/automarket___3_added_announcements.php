	...
	while($row2=GeneralMYSQL::fetch_array($res2)) {
		...
		if ($row2['img']==""){?><img src="http://mapstore.org/my_portfolio/tazteam.net/images/_general/general___photo_none_210x160.png" width="210" height="160" class="refimage"><?php }
			...
			<?php if ($row2['themepage']==1){?>			
				<a href="http://mapstore.org/my_portfolio/tazteam.net/automarket/<?php echo(GeneralGetVars::$var2."/".$row2['id']);?>" class="lead"><?php echo(AutomarketBase::return_parameters("mark", $row2['mark'])." "); echo($row2['model']);?></a>
				<div></div>
...
<div class="v_i_s"></div>
			...
			<?php }
				...
				<a href="http://mapstore.org/my_portfolio/tazteam.net/automarket/<?php echo(GeneralGetVars::$var2."/".$row2['id']);?>" class="lead"><?php echo($row2['name']);?></a>
				<div></div>
