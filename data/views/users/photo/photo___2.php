...
<td class="photo2_3">
	<div class="photo2_4">
		<a href="<?php echo(GeneralGlobalVars::url);?>/photo/<?php echo(GeneralGetVars::$var2."/".$row['id_topic']."=".PhotoBase::$current_num_page_photo);?>" class="big_link_h1"><?php echo($row['name_topic']);?></a>
		<?php if (GeneralGetVars::$var2!=1){/*привязка 1 от галереи*/?>
		<div class="v_i_b_h1"></div>
		<span class="explanation">Автор: </span> <a href="<?php echo(GeneralGlobalVars::url);?>/users/<?php echo($row['id_autor_topic']);?>" class="small_dark_link"><?php echo(UsersMyData::return_name($row['t_login_user'],$row['t_mail_user'],$row['t_name_user'],$row['t_surname_user'],$row['t_login_status']));?></a>
		<?php } ?>
	</div>
</td>
</tr>
<tr>
<td onclick="javascript:location.href='<?php echo(GeneralGlobalVars::url);?>/photo/<?php echo(GeneralGetVars::$var2."/".$row['id_topic']."=".PhotoBase::$current_num_page_photo);?>'"
...
<td class="photo2_3">
	<div class="photo2_9">
		<a href="<?php echo(GeneralGlobalVars::url);?>/photo/<?php echo(GeneralGetVars::$var2."/".$row['id_topic']);?>=1" class="big_link_h1"><?php echo($row['name_topic']);?></a>
		<div class="v_i_b_h1"></div>
		<span class="explanation">Автор: </span> <a href="<?php echo(GeneralGlobalVars::url);?>/users/<?php echo($row['id_autor_topic']);?>" class="small_dark_link"><?php echo(UsersMyData::return_name($row['t_login_user'],$row['t_mail_user'],$row['t_name_user'],$row['t_surname_user'],$row['t_login_status']));?></a>
	</div>
</td>
