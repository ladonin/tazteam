...
while($row=GeneralMYSQL::fetch_array($res)) {
	...
	<td valign="top" align="center">
			...
			<a href="http://instorage.org/portfolio/tazteam/garage/<?php echo($row['themepage']."/".$row['id']);?>" class="lead"><?php echo(GarageBase::return_parameters("mark", $row['mark'])." "); echo($row['model']);?></a>
		...
		<a href="http://instorage.org/portfolio/tazteam/garage/<?php echo($row['themepage']."/".$row['id']);?>"><?php
        if ($row['img']==""){?><img src="http://instorage.org/portfolio/tazteam/images/_general/general___photo_none_300x200.png" width="300" height="200" class="refimage"/><?php }
...
<div class="v_i_s"></div>
	...
	<span class="grey">Владелец: </span> <a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']);?>" class="link_lead "><?php echo(UsersMyData::return_name($row['t_login_user'],$row['t_mail_user'],$row['t_name_user'],$row['t_surname_user'],$row['t_login_status']));?></a>
