...
<?php
?>
<a href="http://instorage.org/portfolio/tazteam/users"><strong>Участники</strong></a><br>
<?php
$res=mysql_query("SELECT id_user FROM registrated_users___main_data order by id_user"); 	
while ($row=mysql_fetch_array($res)) 
{
?>
<a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']);?>">участник <?php echo($row['id_user']);?></a><br>
<?php
}


?>
<a href="http://instorage.org/portfolio/tazteam/users"><strong>Фотоальбомы</strong></a><br>
<?php
$res=mysql_query("SELECT id_user,id_album,page_photo FROM registrated_users___photoalbums_photos"); 	
while ($row=mysql_fetch_array($res)) {
	if ($row['page_photo']>0){
?>
<a href="http://instorage.org/portfolio/tazteam/users/<?php echo($row['id_user']."/photoalbums/".$row['id_album']."=".$row['page_photo']);?>">участник <?php echo($row['id_user']);?>, альбом <?php echo($row['id_album']);?>, фото <?php echo($row['page_photo']);?></a><br>
<?php
}}
?>
</body>
